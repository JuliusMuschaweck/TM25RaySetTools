...
namespace TM25 {
	...
	void TLightToolsRaySet::AddRay(float x, float y, float z, float kx, float ky, float kz, float flux)
		{
		data_.push_back(x);
		data_.push_back(y);
		data_.push_back(z);
	...
	void TLightToolsRaySet::SetRaysDirect(const std::vector<float>& rays, size_t nRays, bool withWavelength)
		{
		nItems_ = withWavelength ? 8 : 7;
		if (rays.size() != nItems_ * nRays)
			throw TM25Error("TLightToolsRaySet::SetRaysDirect: inconsistent ray array size");
		if (withWavelength)
			{
			data_.resize(nItems_ * nRays);
			...
			for (size_t i = 0; i < nRays; ++i)
				...
				float kz = *(cur++);
				float flux = *(cur++);
				AddRay(x, y, z, kx, ky, kz, flux);
				//float lam = this->Wavelength();
				//AddRay(x, y, z, kx, ky, kz, flux, lam);
				}
			}
		}
